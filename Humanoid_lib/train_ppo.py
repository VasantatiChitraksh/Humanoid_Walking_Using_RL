...
from lib.PPO_Agent import PPOAgent
...
def ppo_update(agent, optimizer, scaler, batch_obs, batch_actions, batch_returns, batch_old_log_probs, batch_adv,
    ...
    with torch.amp.autocast(str(device)):
        ...
        _, new_log_probs, entropies, new_values = agent.get_action_and_value(
            batch_obs, batch_actions)
        ratio = torch.exp(new_log_probs - batch_old_log_probs)

        # Approximate mean KL divergence for this batch per action dimension
        kl = ((batch_old_log_probs - new_log_probs) /
              batch_actions.size(-1)).mean()

        # Surrogate objectives
        surr1 = ratio * batch_adv
        surr2 = torch.clamp(ratio, 1.0 - clip_epsilon,
                            1.0 + clip_epsilon) * batch_adv
...
if __name__ == "__main__":
    ...
    test_env = make_env(
        args.env, reward_scaling=args.reward_scale, render=True)
    ...
    # Create the buffer
    buffer = PPOBuffer(obs_dim, act_dim, args.n_steps,
                       args.n_envs, device, args.gamma, args.gae_lambda)
    ...
    next_obs = torch.tensor(
        np.array(envs.reset()[0], dtype=np.float32), device=device)
    next_terminateds = torch.zeros(
        args.n_envs, dtype=torch.float32, device=device)
    next_truncateds = torch.zeros(
        args.n_envs, dtype=torch.float32, device=device)
    reward_list = []
    ...
    try:
        for epoch in range(1, args.n_epochs + 1):
            ...
            for _ in tqdm(range(0, args.n_steps), desc=f"Epoch {epoch}: Collecting trajectories"):
                ...
                next_obs, rewards, next_terminateds, next_truncateds, _ = envs.step(
                    actions.cpu().numpy())

                # parse everything to tensors
                next_obs = torch.tensor(
                    np.array(next_obs, dtype=np.float32), device=device)
                reward_list.extend(rewards)
                rewards = torch.tensor(
                    rewards, dtype=torch.float32, device=device)
                next_terminateds = torch.as_tensor(
                    next_terminateds, dtype=torch.float32, device=device)
                next_truncateds = torch.as_tensor(
                    next_truncateds, dtype=torch.float32, device=device)

                # Store the step in the buffer
                buffer.store(obs, actions, rewards, values,
            ...
            with torch.no_grad():
                ...
                traj_adv, traj_ret = buffer.calculate_advantages(
                    next_values, next_terminateds, next_truncateds)
            ...
            if epoch % args.render_epoch == 0:
                log_video(test_env, agent, device, os.path.join(
                    videos_dir, f"epoch_{epoch}.mp4"))
